...
def trueDikeLength(lines, dikeset, maxL, Lstep=2000, secondbar=False):
    fig,axs=plt.subplots() #(2)
    axs.grid(False)
    
    a=axs
    bins=np.arange(1,maxL,2000)
    a.hist(lines['R_Length'], bins=bins, color='tab:blue', label='Linked Segment Length')
    ...
    if secondbar:
        ...
        a2.hist(dikeset['seg_length'], color='tab:red', label='Single Segment Length')
        a2.set_ylabel('Count of Segments', color='tab:red')
        a2.tick_params(axis='y', labelcolor='tab:red')
        
        
        a2.text( .60,.50 ,'Segment mean:'+str(round(oldavg,0)),transform=a2.transAxes)
        a2.text( .60,.40, 'Segment std:'+str(round(oldstd,0)),transform=a2.transAxes)
...
def DotsHT(dikeset,lines):
    ...
    #ax[0],h1=HThist(dikeset['rho'], dikeset['theta'],rstep, tstep, weights=dikeset['seg_length'], ax=ax[0], rbins=rbins)
    c1=ax[0].scatter(dikeset['theta'], dikeset['rho'], c=dikeset['seg_length'], cmap=cm.plasma, edgecolor='black')
    ax[0].set_title('Raw Data')
    ax[0].set_xlabel('Theta (degrees)')
    ax[1].set_xlabel('Theta (degrees)') 
    ax[0].set_ylabel('Rho (m)')
    ...
    #ax[1], h2=HThist(lines['AvgRho'], lines['AvgTheta'], rstep, tstep, weights=lines['R_Length'], ax=ax[1],rbins=rbins)
    c2=ax[1].scatter(lines['AvgTheta'], lines['AvgRho'], c=lines['R_Length'], cmap=cm.plasma, edgecolor='black')
    ax[1].set_title('Clustered Data')
