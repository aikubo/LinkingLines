...
from syntheticMod import fromHT, fragmentDikes
...
class TestAggCluster:
    ...
    def test_AggCluster_without_rotation(self, synthetic_dikeset):
        ...
        df= fragmentDikes(synthetic_dikeset)
        
        dikeset, Z = AggCluster(df, dtheta=1.0, drho=1.0, rotate=False)
    ...
    def test_AggCluster_with_rotation(self, synthetic_dikeset):
        dikeset, Z = AggCluster(synthetic_dikeset, dtheta=1.0, drho=1.0, rotate=True)
    ...
    def test_AggCluster_with_custom_parameters(self, synthetic_dikeset):
        dikeset, Z = AggCluster(synthetic_dikeset, dtheta=0.5, drho=0.5)
